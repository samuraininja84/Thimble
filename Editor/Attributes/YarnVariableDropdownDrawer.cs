    ...
    public class YarnVariableDropdownDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (!initialized)
            {
                ...
                // Load variables from the specified folder path
                variables = LoadVariables(attr.FolderPath);
            ...
            if (GUI.Button(buttonRect, string.IsNullOrEmpty(currentValue) ? noneOption : currentValue))
            {
                ...
                mousePosition.OpenStringSearchWindow(windowTitle, variables, selectedVariable =>
                {
                    // Update the property value with the selected variable without the type annotation
                    property.stringValue = GetVariableName(selectedVariable);
        ...
        /// <param name="folderPath">The path to the folder containing Yarn files to be processed. This folder and its subdirectories will be searched for files with a ".yarn" extension.</param>
        /// <returns>A sorted list of unique variables found in the Yarn files, including their types. The list begins with a "<None>" entry.</returns>
        ...
        private string GetVariableName(string variable)
        {
            ...
            if (typeIndex > 0) variable = variable.Substring(0, typeIndex);
        ...
        private string GetVariableType(string line)
        {
            ...
            // If a match is found, analyze the captured type
            if (match.Success)
            {
