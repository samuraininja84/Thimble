...
{
    [CustomPropertyDrawer(typeof(CompositeBoolVariable))]
    public class CompositeBoolVariablePropertyDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var nameProperty = property.FindPropertyRelative(nameof(CompositeBoolVariable.Name).ToLower());

            // Get the value property and set it to lowercase to match the serialized field name
            var valueProperty = property.FindPropertyRelative(nameof(CompositeBoolVariable.Value).ToLower());
            ...
            // Width for the bool field
            float boolFieldWidth = 20f;

            // Calculate the width for the name and value properties
            Rect nameRect = new Rect(position.x, position.y, position.width - boolFieldWidth - gap, position.height);

            // Adjust the value rect to accommodate the smaller field for bool
            Rect valueRect = new Rect(position.x + gap + position.width - boolFieldWidth, position.y, boolFieldWidth, position.height);

            // Draw name and value properties
            EditorVariableExtensions.DrawVariableSelector(nameRect, nameProperty, valueProperty, VariableType.Bool, true);
